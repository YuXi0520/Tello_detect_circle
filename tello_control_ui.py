...
class TelloUI:
    ...
    def parse_circle(self,location):
        mylist = []
        num_posi = []
        for s in location.replace(': ', '|').split('|'):
            mylist.append(s)
        # print mylist[1], mylist[3], mylist[5]
        num_posi.append(int(mylist[1]))
        num_posi.append(int(mylist[3]))
        num_posi.append(int(mylist[5]))
        if len(num_posi) < 2:
            return False
        if num_posi[0]==0 :
            return False
            ...
            while not self.stopEvent.is_set():
                ...
                if self.pose_mode:
                    ...
                    if(cmd=="NULL" ):
                        ...
                        circle_posi = self.parse_circle(cmd)
                        cmd = ""
                    if(circle_posi):
                        if(self.posi_flag==1):
                            self.Old1=circle_posi
                            self.posi_flag+=1
                            cmd=""
                            print("OLD1")
                        #----------------------------------------
                        if(self.posi_flag==2):
                            #if (abs(self.Old1[0] - circle_posi[0]) < 20 & abs(self.Old1[1] - circle_posi[1]) < 20):
                            self.Old2=circle_posi
                            print("OLD2:")
                            self.posi_flag=1#还原为1
                            if(self.Old2[0]>550):#x:在画面左边
                                cmd = 'moveright'
                            elif (self.Old2[0] < 470):  #x:在画面右边
                                cmd = 'moveleft'
                            elif (self.Old2[1] > 250):  #y:在画面下边
                                cmd = 'telloDown'
                            elif (self.Old2[1] < 170):  #y:在画面上边
                                cmd = 'telloUp'
                            else :
                                cmd = 'moveforward'
                                print("wancheng!!!!!!!")
                ...
                if cmd == 'moveback':
                    ...
                    self.telloMoveLeft(0.20)
                    time.sleep(2)
                elif cmd == 'moveright':
                    self.telloMoveRight(0.20)
                    time.sleep(2)
                elif cmd == 'telloUp':
                    self.telloUp(0.20)
                    time.sleep(2)
